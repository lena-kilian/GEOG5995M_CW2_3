...
region_index.to_csv('region_index.csv', sep = '\t')
...
country_data = country_filter.join(cba_country.append(wdi_country)).join(country_index.drop(['region'], axis = 1)).join(region_index, on = 'region_code').sort_index(axis=1, ascending=True).sort_values(by = ['code', 'country_list'], ascending=True).drop('short_name', axis = 1)
